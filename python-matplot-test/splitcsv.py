...
def main():
   ...
   # splitpath = int(input("請選擇csv工作資料夾 \n"))
    ...
    if splitmodel == 1:
     # 搜尋指定資料夾下的所有 .csv 檔案
     csv_Allfiles = [f for f in os.listdir(folder_path) if f.endswith('.csv')]

    # 如果資料夾下沒有 .csv 檔案，則退出
     if not csv_Allfiles:
        print("目前選擇資料夾下沒有 .csv 檔案，程序退出。")
        exit()

     # 依次處理每個 CSV 檔案
     for csv_file in csv_Allfiles:
            file_path = os.path.join(folder_path, csv_file)

            # 將檔案名稱加入列表
            all_csvfiles.append(csv_file)
            ...
            # 新檔案名稱
            base_filename = os.path.splitext(csv_file)[0]  # 取得原檔名，不含副檔名
            ...
            print(f"檔案 {csv_file} 已被拆分並儲存為三個檔案。")
    ...
    #使用讀寫csv 套件擷取檔案依照row數量拆分
     ...
     for csv_file in csv_Allfiles:
         ...
         # 將檔案名稱加入列表
         all_csvfiles.append(csv_file)
         origin_file_path = os.path.join(folder_path, csv_file)
        ...
        # 以檢測到的編碼讀取檔案
         ...
         print(f"檔案 {csv_file} 已被拆分{file_number}個檔案。")
